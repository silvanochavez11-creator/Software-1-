...
/* ---- WhatsApp de ventas de Aivoraia (botón flotante de la web pública) ---- */
const VENTAS_WHATSAPP = "5219622526823"; // +52 962 252 6823
const VENTAS_WA_LINK = `https://wa.me/${VENTAS_WHATSAPP}?text=${encodeURIComponent("Hola, quiero probar Aivoraia para mi refaccionaria 🏍️")}`;

/* ---- Píxel de Meta (anuncios de Facebook/Instagram) ----
   Cuando crees tu píxel en el Administrador de Eventos de Meta, pega su ID
...
export default function RefaccionariaSaaS() {
  ...
  useEffect(() => {
    initMetaPixel(); // píxel de Meta para los anuncios (no hace nada sin ID)
    (async () => {
...
function LandingPage({ onEnter }) {
  ...
  return (
    <div style={{ "--accent": DEFAULT_ACCENT, "--accent-soft": DEFAULT_ACCENT + "22", minHeight: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      {/* ---- WhatsApp de ventas (flotante) ---- */}
      <a href={VENTAS_WA_LINK} target="_blank" rel="noreferrer" onClick={() => fbTrack("track", "Contact")}
        title="Escríbenos por WhatsApp"
        style={{ position: "fixed", right: 16, bottom: 16, zIndex: 60, display: "flex", alignItems: "center", gap: 9, background: "#25D366", color: "#fff", borderRadius: 999, padding: "13px 19px", fontWeight: 700, fontSize: 13, textDecoration: "none", boxShadow: "0 10px 30px #00000066" }}>
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "login") {
        ...
        fbTrack("track", "CompleteRegistration"); // conversión para los anuncios de Meta
...
function CreateOrgScreen({ email, onCreated, onSignOut, onRetry }) {
  ...
  const create = async () => {
    ...
    try {
      ...
      // conversión para los anuncios de Meta
      if (trial) fbTrack("track", "StartTrial", { value: 999, currency: "MXN", predicted_ltv: 999 });
      else fbTrack("trackCustom", "NegocioCreado");
      await onCreated();
